...
function analyzePlus(data) {
  ...
  const html = [
    ...
    `量能：${volNote}  BOLL带宽：${bw ? (bw * 100).toFixed(1) + '%' : '—'} ${T('boll')}  ATR14：${atrNow.toFixed(2)} ${T('atr')}`,
  ...
  try {
    ...
    if (!shIndexData) {
      fetchJson(urlDayK('sh000001')).then(j2 => {
        ...
        $('advanced').innerHTML = computeAdvanced(data, shIndexData, basic);
      }).catch(() => {
        $('advanced').innerHTML = computeAdvanced(data, null, basic);
      });
    } else {
      $('advanced').innerHTML = computeAdvanced(data, shIndexData, basic);
    }
